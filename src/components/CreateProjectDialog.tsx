...
export function CreateProjectDialog({
  ...
  const handleCreateProject = () => {
    ...
    if (tracksWithoutTags.length > 0) {
      alert('Please assign a tag to all tracks');
      return;
    }
                      ...
                      {authors.map((author) => (
                        <TableRow key={author.id}>
                          ...
                          <TableCell>
                            <Select
                              value={author.role}
                              onValueChange={(value) => handleAuthorChange(author.id, 'role', value)}
                            >
                              <SelectTrigger>
                                <SelectValue />
                              </SelectTrigger>
                    ...
                    {audioTracks.map((track, index) => (
                      <TableRow key={index}>
                        ...
                        <TableCell>
                          <Select 
                            value={track.tag || ''} 
                            onValueChange={(value) => handleTagChange(index, value as TrackTag)}
                          >
